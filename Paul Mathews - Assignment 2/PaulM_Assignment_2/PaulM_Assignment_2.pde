...
triForce[] movers = new triForce[10];//array of things to move
...
void draw() {
  ...
  for (int i = 0; i < movers.length; i++) {
    ...
    for(int j = 0; j < movers.length; j++){
      if(j != i){
        movers[i].edgeCheck(movers[j].getLocationV()); //check all the edges
      }
    }        
  }
...
void oscSendVel() {
  ...
  for (int i = 0; i < movers.length; i++) {
    myMessage.add(movers[i].getVelocity());//fill message with velocities
  }
